  ...
  return (
    <div className="mt-2 flex flex-wrap gap-2">
      {refs.map((r, idx) => (
        <span
          key={`${r.file_name}-${idx}`}
          className="text-xs px-2 py-1 rounded-full bg-white/70 text-gray-800 border border-gray-200 backdrop-blur"
        >
          📄 {r.file_name}
        </span>
      ))}
      ...
      <div className="flex-1 overflow-y-auto rounded-2xl border border-white/10 bg-white/5 p-4 space-y-4 backdrop-blur">
        ...
        <AnimatePresence initial={false}>
          {messages.map((m) => (
            ...
            >
              <div
                className={`max-w-xl px-4 py-3 rounded-2xl shadow-lg ${m.role === 'user' ? 'bg-gradient-to-r from-purple-600 to-pink-600 text-white' : 'bg-white text-gray-900'}`}
              >
                {m.content}
